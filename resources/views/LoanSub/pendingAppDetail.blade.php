...
<div class="container">
    ...
    <div class="row">
        <div class="col s12 subject-header">
            <span class="teal-text">PENDING LOAN DETAIL</span>
        </div>
    </div>
    <div class="row">
        <div class="col s12 subject-header">
    ...
    <div class="row user-profiles">
        ...
        <div class="col s12 m3 l3 profile">
            ...
            <span class="profile__user-name">Amount Req {{number_format($userLoan->amount_applied,2,'.',',')}} </span>
        </div>
        <div class="col s12 m3 l3 profile">
            ...
            <span class="profile__user-name">Req 30%
                {{number_format($userLoan->user->requiredPercent($userLoan->amount_applied), 2,'.',',')}}
            </span>
            <span class="profile__user-name">Avail 30%
                {{number_format($userLoan->user->availablePercent($userLoan->user_id), 2,'.',',')}}</</span> </div> <div
                    class="col s12 m3 l3 profile">
                <p class="profile__heading text-grey darken-3">STATUS</p>
                <span><i class="small material-icons pink-text lighten-4">looks</i></span>
                <span class="profile__user-name">{{$userLoan->loan_status}}</span>

                <span class="profile__user-name">Application Date</span>
                <span class="profile__user-name">{{$userLoan->created_at->toFormattedDateString()}}</span>
