...
<div class="container">
    ...
    <div class="row user-profiles">
        ...
        <div class="col s12 m3 l3 profile">
            ...
            <span class="profile__user-name">Unit Cost N {{number_format($review->product->unit_cost,2,'.','.')}}</span>
        </div>
        <div class="col s12 m3 l3 profile">
            <p class="profile__heading text-grey darken-3">PRODUCT SUMMARY</p>
            <p><i class="small material-icons pink-text lighten-4">looks</i></p>
            <span class="profile__user-name">Units {{$review->units}} (Total N {{number_format($review->total_amount,2,'.',',')}})</span>
            <span class="profile__user-name">Repayment N {{number_format($review->monthly_repayment,2,'.',',')}}</span>
        </div>

        <div class="col s12 m3 l3 profile">
            <p class="profile__heading text-grey darken-3">STATUS</p>
            <p><i class="small material-icons pink-text lighten-4">looks</i></p>
            <span class="profile__user-name">{{$review->status}}</span>
    ...
    <div class="row">
        <form class="col s12" method="POST" action="/prodSub/reviewStore/{{$review->id}}">
            {{ csrf_field() }}

            <div class="row">
                <div class="input-field col s12 m3 l3">
                    <input id="units" name="units" type="text" value="{{$review->units}}" class="validate">
